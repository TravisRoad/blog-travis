...
publishDate: 2023-01-20 02:44:56
...
<Dialog reverse="true">
  <p className="not-prose">
    ...
    <del>
      <a
        href="https://wiki.archlinux.org/title/Arch_is_the_best"
        className="text-nord-10 hover:underline dark:text-nord-9"
      >
        <>"Arch is the best!"</>
      </a>
    </del>
...
Arch Linux 有很齐全的 [ArchWiki][wiki] 可供查阅。

学习所有东西都是有成本的，即使是友好的 Windows 系统，小朋友也需要上手慢慢使用才能逐渐弄明白操作逻辑。
在本科期间，第一次接触 Linux 是在 CSAPP 课程的 bomb lab 上，当时连 shell 都不知道怎么用，需要使用 gdb 调试器在命令行中敲敲打打拆炸弹。这是对 Linux 的直观影响。
实践之后需要理论学习，将之内化于心。要学习 Linux，除了看到的各种小技巧，还需要大量的实践来熟悉操作。
接下来 “熟悉” 和 “理解” 之间还有很多的路需要走，这个时候就需要再去学习理论知识，如此循环往复，是一个螺旋上升的过程。

为了大量实践 Linux, 日常使用 Linux 是一个好的选择，不是使用虚拟机，不是使用远程服务器，而是真真正正自己安装一个系统在物理磁盘上，从 boot 开始一步步配置的 Linux 系统。
在这个过程中，不出意外的会遇到了许多挫折，很多 “难者不会，会者不难” 的问题能折腾许久，
通过 [RTFM][rtfm]，[STFW][stfw] 最后就能解决 90% 的问题。
...
[^tilingwm]: [tiling windows manager](https://wiki.archlinux.org/title/Window_manager#Tiling_window_managers)
...
至于游戏方面，阀门厂的 Steam Deck 上的 SteamOS [^steamos] 就是 Arch 系的，阀门厂为兼容层做了相当多的优化，相信未来 Linux 的游戏生态也会有所好转。
...
不够开箱即用，虽然可以自定义很多组件，不过都需要折腾。对于小白来说，最难的问题莫过于定位问题，没有碰到过千奇百怪的问题就很难知道到底是什么组件出现了问题。
我个人而言，就遇到过 GRUB 引导错误，X server 不能启动等问题，身边常备着 [ventoy](https://www.ventoy.net/cn/index.html) 的 U 盘，里面装着最新的 Arch Live 光盘映像，随时准备修复系统。

Arch Linux 是滚动发行版，不同于 ubuntu 的点发行版，Arch Linux 更新很频繁，网传有所谓的 “滚挂” 的现象产生。

## 总结

适合自己的才是最好的。Arch Linux 给你提供了构建适合自己的工作流的途径，就像在搭积木一样一点点搭起来。
不想折腾的同学大可以使用开箱即用的操作系统。

如果读者看到这里想自己试试装个 Linux 玩玩。
不妨从 Ubuntu desktop 开始试水，然后再试试 [Manjaro](https://manjaro.org/)，这是一个 arch 系的操作系统，开箱即用，可以轻松体验到 arch 生态。
如果你想直接入手 Arch Linux，不妨从官网的 [安装指南](https://wiki.archlinux.org/title/Installation_guide) 开始，也可以使用 Arch Linux Studio 发布的 [ArchTutorial](https://archlinuxstudio.github.io/ArchLinuxTutorial/#/)。

不管是哪个发行版，遇到问题时时刻谨记 RTFM、STFW、~~RTFSC~~，大部分情况都会有前人替我们踩好了一路上的坑，自己实在解决不了了再寻求社区帮助，或者给仓库提 issue。
