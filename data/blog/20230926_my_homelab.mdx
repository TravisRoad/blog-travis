...
这里我在 wrt 后面配置了三台虚拟机用于跑 k8s，具体实现则使用了 [k3s](https://k3s.io) 进行部署和管理。k3s 默认配置了 Local Path Provisioner 的 PVC 方案和 Traefik Ingress Controller，总之就是挺方便的。

而为了访问方便，我将 node0 上的 80/443 端口穿到了外面的和校园网直接互通的端口上。其实这样有点奇怪，更好的做法其实是起一个 vpn，把集群的机器加入虚拟的局域网中，这样更加安全。
之后再考虑考虑吧。
...
dns 的分发上，我做的比较邪道，由于我所有的机器的梯子用的都是我自定义的 clash 文件，所以我在 clash 的 host 选项中定义了域名和 ip 地址解析。
