  ...
  let object = {
    ...
    init() {
      ...
      if (this.state.width < 720) {
        this.state.height = window.innerHeight / 2;
      }
      ...
      Array.from(this.props.textContainer.querySelectorAll("img")).forEach(
        (img) => {
    ...
    setupBreakpoints(path) {
      return this.points
        .map((point) => Path.getLengthAtPoint(path, point))
        .map((point, i) =>
          this.sections[i].getAttribute("data-stay") == "true"
            ? [point, point]
            : [point]
        )
    ...
    updateScroll(scroll) {
      ...
      let cameraSegment = {
        start:
          this.cameraBreakpoints[
            clamp(sectionIndex, this.cameraBreakpoints.length - 1)
          ],
      ...
      let trailSegment = {
        start:
          this.trailBreakpoints[
            clamp(sectionIndex, this.trailBreakpoints.length - 1)
          ],
    ...
    getMapSliceAtPercent(percent) {
      ...
      let center = {
        ...
        y: this.state.width > 720 ? 0.43 : 0.5,
      };
      return {
    ...
    getZoomAtPercent(percent) {
      ...
      let lastSection =
        this.sections[clamp(sectionIndex - 1, 0, this.sections.length - 1)];
    ...
    renderMap() {
      ...
      let drawImagePointer = (image) => {
        ...
        let drawTriangle = (corner1, corner2) => {
          ...
          this.ctx.beginPath();
          this.ctx.moveTo(origin.x + originOffset.x, origin.y + originOffset.y);
          this.ctx.lineTo(...corner1);
          this.ctx.lineTo(...corner2);
          this.ctx.lineTo(origin.x - originOffset.x, origin.y - originOffset.y);
      ...
      let drawLabel = (point, i) => {
        let fontSize = 15;
        this.ctx.font = `${setByStatus(i, "normal", "bold")} ${setByStatus(
          i,
          fontSize,
          fontSize * 1.2
        )}px Arial`;
      ...
      let drawMap = () => {
        ...
        if (!updatedBufferThisFrame) {
          ...
          drawCanvasSlice(this.ctx, this.mapBuffer, slice, target);
        } else {
          this.ctx.drawImage(
            this.mapBuffer,
      ...
      const createT2BLinearGradient = () => {
        ...
        let gradient = this.ctx.createLinearGradient(
          ...
          0, // 起点 y 坐标为 0
          0,
          (this.state.height * 2) / 3 // 终点 y 坐标为画布高度
        );
      ...
      // createL2RLinearGradient();
      if (this.state.width < 720) {
        createT2BLinearGradient();
      }
    },
    render() {
      this.renderMap();
